...
use crate::bucket::lifecycle::tier_sweeper::{RemoteTierDeleteOutcome, delete_object_from_remote_tier_idempotent};
...
pub(crate) async fn cleanup_uncommitted_transition_upload(
    ...
    delete_object_from_remote_tier_idempotent(object, candidate.cleanup_version(), tier).await
}

fn log_transition_upload_cleanup_failure(tier: &str, object: &str, candidate: &TransitionUploadCandidate, err: &std::io::Error) {
    warn!(
        tier,
        object,
...
impl TransitionUploadCleanup {
    pub(crate) fn new(tier: &str, object: &str, candidate: TransitionUploadCandidate) -> Self {
        Self {
            tier: tier.to_string(),
            object: object.to_string(),
            candidate,
    ...
    pub(crate) async fn cleanup(&mut self) -> std::io::Result<RemoteTierDeleteOutcome> {
        match cleanup_uncommitted_transition_upload(&self.tier, &self.object, &self.candidate).await {
            Ok(outcome) => {
                self.armed = false;
                Ok(outcome)
            }
            Err(err) => {
                log_transition_upload_cleanup_failure(&self.tier, &self.object, &self.candidate, &err);
                Err(err)
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            handle.spawn(async move {
                if let Err(err) = cleanup_uncommitted_transition_upload(&tier, &object, &candidate).await {
                    log_transition_upload_cleanup_failure(&tier, &object, &candidate, &err);
                }
            });
        }
    }
        ...
        let candidate = match rv {
            ...
            Err(failure) => {
                if let Some(candidate) = failure.candidate {
                    let mut upload_cleanup = TransitionUploadCleanup::new(&opts.transition.tier, &dest_obj, candidate);
